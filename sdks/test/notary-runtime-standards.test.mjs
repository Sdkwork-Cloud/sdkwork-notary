...
test("notary Rust runtime crates declare SDKWork component metadata", () => {
  for (const crateName of [
    ...
    "sdkwork-routes-notary-http-auth",
    ...
    } else if (crateName === "sdkwork-routes-notary-http-auth") {
      assert(spec.canonicalSpecs.some((entry) => entry.file === "WEB_FRAMEWORK_SPEC.md"));
    } else if (crateName.startsWith("sdkwork-routes-notary-")) {
      assert(spec.canonicalSpecs.some((entry) => entry.file === "WEB_FRAMEWORK_SPEC.md"));
...
test("workspace verification includes contract tests and Rust runtime tests", () => {
  ...
  assert.equal(
    ...
    "node --test sdks/test/*.test.mjs scripts/dev/*.test.mjs scripts/verify-notary-standard-architecture.test.mjs scripts/verify-notary-iam-manifest.test.mjs scripts/verify-notary-utils-standard.test.mjs apps/sdkwork-notary-h5/src/__tests__/*.test.mjs apps/sdkwork-notary-pc/src/__tests__/*.test.mjs tests/contract/*.test.mjs",
  );
  assert.equal(
