use sdkwork_api_notary_assembly::assemble_api_router;
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let bind_address = std::env::var("SDKWORK_NOTARY_APPLICATION_PUBLIC_INGRESS_BIND")
    ...
    let app = ComposedApiAssembly::try_compose("SDKWork Notary API", vec![assembly])
        .map_err(std::io::Error::other)?
        .into_hosted(framework)
        .router;
    let bind_address = bind_address.parse()?;
    println!("sdkwork-api-notary-standalone-gateway listening on http://{bind_address}");
