...
export interface NotaryApiPort {
  ...
  createCase(input: Parameters<NotaryH5ComposedApi["createCase"]>[0]): Promise<unknown>;
  uploadCaseFile(input: Parameters<NotaryH5ComposedApi["uploadCaseFile"]>[0]): Promise<unknown>;
  attachPartySignature(
    caseId: string,
    partyId: string,
    input: Parameters<NotaryH5ComposedApi["attachPartySignature"]>[2],
  ): Promise<unknown>;
  createPartyVideoInvite(
    caseId: string,
    partyId: string,
    input?: Parameters<NotaryH5ComposedApi["createPartyVideoInvite"]>[2],
  ): Promise<unknown>;
...
export function createNotaryService(
  resolveApi: () => NotaryApiPort = getNotaryH5ComposedApi,
) {
  return {
    getRecordTabs() {
      return RECORD_TABS;
